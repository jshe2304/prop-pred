...
class MLPRegressor(BaseMLP):
    ...
    def scoring_function(self, y, y_pred):
        return r2_score(y.detach().cpu(), y_pred.detach().cpu())
...
class MLPClassifier(MLPRegressor):
    ...
    def scoring_function(self, y, y_pred):
        return roc_auc_score(y.detach().cpu(), y_pred.detach().cpu(), average=None).mean()
